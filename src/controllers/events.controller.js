...
const createEvent = asyncHandler(async (req, res) => {
  ...
  if (!eventImages.length)  throw new ApiError(500, "Something Went Wrong While Uploading Images!");
...
const getAllEvents = asyncHandler(async (req, res) => {
  ...
  if(id) {
    ...
    return res.status(200).json(new ApiResponse(200, true, "Event Fetched Successfully!", event));
  }
  ...
  await db.collection("events").aggregate([
    {
      $facet: {
        ...
        data: [{ $skip: (page - 1) * pageSize }, { $limit: pageSize }, {$sort: {schedule: sort}}],
      },
    },
  ...
  res.status(201).json(new ApiResponse(201, true, "Events Fetched Successfully!", events[0]));
});
...
const updateEvent = asyncHandler(async (req,res)=>{
  ...
  const eventDetails = await db.collection("events").findOne({_id: ObjectId.createFromHexString(id)});
  ...
  const updatedEvent = await db.collection("events").findOneAndUpdate({_id: ObjectId.createFromHexString(id)},{$set: {...req.body}}, {returnDocument: "after"});

  if (!updateEvent) return new ApiResponse(200, true, "No Events Found!");

  res.status(200).json(new ApiResponse(200, true, "Event Fetched Successfully!", updatedEvent));
});
...
const deleteEvent = asyncHandler(async (req,res)=>{
  ...
  const event = await db.collection("events").findOneAndDelete({_id: ObjectId.createFromHexString(id)});
  
  if (!event) return new ApiResponse(200, true, "No Events Found!");

  res.status(200).json(new ApiResponse(200, true, "Event Deleted Successfully!", event));
});
